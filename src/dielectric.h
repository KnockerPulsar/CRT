...
float dielectric_reflectence(Dielectric dielec, float cosine, float ref_idx) {
...
bool dielectric_scatter(Dielectric dielec, const Ray* r_in, const HitRecord* rec, float3* attenuation, Ray* scattered, uint2* seed) {
  *attenuation = (float3){1, 1, 1};
  ...
  float refraction_ratio = rec->front_face? (1/dielec.ir) : dielec.ir;
  ...
  bool random_reflection = dielectric_reflectence(dielec, cos_theta, refraction_ratio) > random_float(seed);
